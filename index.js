...
function findElementGreaterThan(arr, num) {
  ...
  const newElem = arr.find((element) => element > num);

  if (newElem) {
    return newElem;
  } else {
    return `умову не знайдено`;
  }
}
...
function findIndexByProperty(arr, name) {
  ...
  const foundIndex = arr.findIndex((element, name) => name);
...
function getPositiveArrayValues(arr) {
  ...
  const newArr = Array.from(itarator);

  const filteredNewArray = newArr.filter((element) => element > 0);
...
function sumAndPrint(arr) {
  ...
  const sumElem = arr.reduce((accumulator, element) => accumulator + element);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const squaredArr = flatArray.map((element) => element ** 2);

  const filteredArr = squaredArr.filter((element) => element > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const reduceArr = sliceArr.reduceRight(
    (accumulator, element) => accumulator + element
  );

  sliceArr.forEach((n) => console.log(n));
...
function expandArrayByN(arr, n) {
  ...
  const newArr = arr.flatMap((number) => Array(n).fill(number));
...
function calculateTotalPrice(arr) {
  ...
  const totalPrice = arr.reduce((accumulator, currentProduct) => {
